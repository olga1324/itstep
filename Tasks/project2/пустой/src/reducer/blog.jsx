...
/*
        case "BASKET_ADD":
            let newBasketId = ++state.lastShopId;
            let newBasket = {
                id: newBasketId,
                title: action.shop.title,
                description: action.shop.description,
                img: action.shop.img,
                price:action.shop.price,
                quantity:action.shop.quantity,

            };
            return {
                ...state,
               // basket: state.cart.find(({ id }) => id === shop.id);
                basket: [...state.basket, newBasket],
                lastBasketId: newBasketId
            };
        case "BASKET_DELETE":
            return {
                ...state,
                basket: state.basket.filter((item) => item.id != action.basketId)
            }
